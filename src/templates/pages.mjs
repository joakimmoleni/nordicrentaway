...
function header(ctx) {
  ...
  <div class="topline"><div class="wrap"><span>${e(ctx.ui.ownerLabel)}</span><span>Costa Blanca · Spain</span></div></div>
  <header class="site-header"><div class="wrap header-inner">${branding(ctx)}
    ...
    <a class="button header-cta" href="${e(ctx.href('contact/index.html'))}">${e(ctx.ui.primaryCta)}</a><a class="guest-link" href="${e(ctx.site.bookingUrl)}">${e(ctx.ui.guestCta)} ${arrow}</a>
...
function contactBand(ctx) {
 return `<section class="contact-band" aria-labelledby="contact-heading"><div class="wrap contact-band-grid"><div><p class="eyebrow">Your next step</p><h2 id="contact-heading">${lines(ctx.ui.footerHeading)}</h2></div><div><p>${e(ctx.ui.footerText)}</p><a class="button button-light" href="${e(ctx.href('contact/index.html'))}">${e(ctx.ui.primaryCta)} ${arrowRight}</a><a class="band-phone" href="${e(phoneHref(ctx.owner.phone))}">Johan · ${e(ctx.owner.phone)}</a></div></div></section>`;
}
function footer(ctx) {
...
<body>${header(ctx)}<main id="main" tabindex="-1">${content}${contact?contactBand(ctx):''}</main>${footer(ctx)}</body></html>\n`;
}
...
function home(model) {
 ...
 const body=`<section class="wrap hero" aria-labelledby="hero-heading"><div class="hero-copy"><p class="eyebrow">${e(p.eyebrow)}</p><h1 id="hero-heading">${e(p.heading)}<em>${e(p.headingAccent)}</em></h1><p class="lead">${e(p.intro)}</p><div class="hero-actions"><a class="button" href="${e(ctx.href('contact/index.html'))}">${e(p.cta)} ${arrowRight}</a><a class="text-link" href="#how-it-works">${e(p.secondaryCta)} <span aria-hidden="true">↓</span></a></div><p class="hero-contact">Talk directly to Johan <a href="${e(phoneHref(ctx.owner.phone))}">${e(ctx.owner.phone)}</a></p></div><figure class="hero-image">${picture(media,ctx,{hero:true,sizes:'(max-width: 767px) calc(100vw - 40px), 56vw',className:'hero-media'})}<figcaption><span>${e(media?.caption.en || 'Approved photograph needed')}</span><span>${e(p.heroNote)}</span></figcaption></figure></section>
 <section class="services-section" id="services"><div class="wrap split-section"><div class="section-intro"><p class="eyebrow">${e(p.servicesEyebrow)}</p><h2>${lines(p.servicesHeading)}</h2><p>${e(p.servicesIntro)}</p><a class="text-link" href="${e(ctx.href('services/index.html'))}">Our services ${arrowRight}</a></div>${serviceRows(ctx)}</div></section>
 <section class="process-section" id="how-it-works"><div class="wrap"><div class="process-top"><div><p class="eyebrow">${e(p.processEyebrow)}</p><h2>${lines(p.processHeading)}</h2></div><div><p>${e(p.processIntro)}</p><p class="draft-note">${e(p.processNote)}</p></div></div><ol class="process-list">${p.processSteps.map((step,i)=>`<li><span class="step-number" aria-hidden="true">${String(i+1).padStart(2,'0')}</span><h3>${e(step.title)}</h3><p>${e(step.text)}</p></li>`).join('')}</ol></div></section>
 <section class="wrap properties-section" id="properties"><div class="section-heading"><div><p class="eyebrow">${e(p.propertiesEyebrow)}</p><h2>${e(p.propertiesHeading)}</h2><p>${e(p.propertiesIntro)}</p></div><a class="text-link" href="${e(ctx.href('properties/index.html'))}">${e(p.propertiesLink)} ${arrowRight}</a></div>${featured.length?`<div class="properties-grid">${featured.slice(0,3).map((x,i)=>propertyCard(x,ctx,i)).join('')}</div>`:`<p>${e(model.pages.properties.en.emptyText)}</p>`}</section>
 <section class="team-section" id="about"><div class="wrap team-grid"><div class="team-copy"><p class="eyebrow">${e(p.teamEyebrow)}</p><h2>${lines(p.teamHeading)}</h2><p>${e(p.teamIntro)}</p><a class="text-link" href="${e(ctx.href('about/index.html'))}">${e(p.teamLink)} ${arrowRight}</a></div><div class="team-contact"><p class="small-label">Your first point of contact</p>${person(ctx.owner,ctx,{compact:true})}<p class="small-text">Direct contact for a first conversation about your property.</p></div></div></section>
 <section class="area-section"><div class="wrap area-grid"><div><p class="eyebrow">${e(p.areaEyebrow)}</p><h2>${e(p.areaHeading)}</h2><p>${e(p.areaIntro)}</p></div><div class="area-context"><div class="area-line" aria-label="Service area: from Dénia to Orihuela"><span>Dénia</span><span class="area-rule" aria-hidden="true"></span><span>Orihuela</span></div><p>${e(p.areaSecondary)}</p><a class="text-link" href="${e(model.site.recommendationsUrl)}">${e(p.areaLink)} ${arrow}</a></div></div></section>${videoBlock(model,ctx)}`;
...
function contact(model) {
 ...
 return [route,layout(model,route,p,`${pageIntro(p)}<section class="wrap contact-content"><div class="owner-contact"><p class="eyebrow">Your property contact</p><h2>${e(ctx.owner.name)}</h2><a class="contact-email" href="mailto:${e(ctx.owner.email)}">${e(ctx.owner.email)} ${arrow}</a><a class="contact-phone" href="${e(phoneHref(ctx.owner.phone))}">${e(ctx.owner.phone)}</a><div class="contact-actions"><a class="button" href="mailto:${e(ctx.owner.email)}">${e(p.emailCta)} ${arrowRight}</a><a class="button button-outline" href="${e(phoneHref(ctx.owner.phone))}">${e(p.callCta)}</a></div><p class="small-text">${e(p.emailNote)}</p></div><aside class="contact-guidance"><h2>${e(p.helpHeading)}</h2><p>${e(p.helpText)}</p><div class="guest-support"><p class="eyebrow">Guest enquiries</p><h3>${e(p.guestHeading)}</h3><p>${e(p.guestText)}</p><a href="mailto:${e(g.email)}">${e(g.email)}</a><a href="${e(phoneHref(g.phone))}">${e(g.phone)}</a><a class="text-link" href="${e(ctx.site.bookingUrl)}">${e(ctx.ui.guestCta)} ${arrow}</a></div></aside></section>${preparedForm}`,{contact:false})];
}
